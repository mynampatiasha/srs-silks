...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    // Decrease product stock based on quantity
...
router.put('/:id/return', authenticateToken, async (req, res) => {
  try {
    const { returnImage, returnImages, returnReason } = req.body;
    
    const order = await Order.findOne({ _id: req.params.id, customer: req.user.id });
    if (!order) return res.status(404).json({ message: 'Order not found' });

    order.status = 'Return Requested';
    if (returnImage) order.returnImage = returnImage; // Legacy support
